...
fn main() -> eyre::Result<()> {
    ...
    let factory = EthereumNode::provider_factory_builder()
        .open_read_only(chain_spec.clone().into(), ReadOnlyConfig::from_datadir(datadir))?;

    let provider = factory.provider()?;

    let block = provider
        .recovered_block(BlockHashOrNumber::Number(23175593), Default::default())
        .unwrap()
        .unwrap();
    ...
    let block_executor = evm_config.executor(StateProviderDatabase::new(state_provider));
    ...
    let witness = FlatExecutionWitness::new(pre_state, block_hashes, parent);
